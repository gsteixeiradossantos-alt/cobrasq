-- ROLLBACK — FASE B (2026-06-15b_view_casos_sobre_cobrancas.sql)
-- Restaura a view `casos` devedor-cêntrica (definição 2026-06-11b) e os triggers
-- que gravavam em devedores. Rode ANTES do rollback da FASE A.
...
-- NOTA: os triggers fn_casos_insert/update/delete reescritos pela Fase B passam a
-- falhar contra esta view (referenciam cobrancas). Restaure-os a partir de
-- 2026-05-11i (insert), 2026-05-11a (update) e da definição original do CRM
-- (delete = DELETE FROM devedores WHERE id = OLD.id) se for reverter o uso de
-- escrita pela view. Para a maioria dos rollbacks basta o rollback da Fase A +
-- esta redefinição de leitura.
